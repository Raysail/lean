...
class User_model extends CI_Model {
	...
	function select_query( $data, $table_name, $where_condition =false,$order_by_field=false, $order_by_type=false){
		...
		function select_query_with_limit( $data, $table_name, $where_condition =false,$limit,$order_by_field=false, $order_by_type=false){
	...
	function update( $tbl_name , $postdata,  $where ){
	...
	public function check_booking($tbl_name,$where =false)
	{
		$this->db->select('*');
		$this->db->from($tbl_name);
	...
	public function check_no_rec($tbl_name,$where =false,$group_by_field=false)
	{
		$this->db->select('*');
		$this->db->from($tbl_name);
	...
	/* FOR CALENDER END FUNCTION */
	 ...
	 public function get_row_with_con($table,$where,$order_by_field=false, $order_by_type=false)
	 {
	 	$this->db->select('*');
		$this->db->from($table);
		$this->db->where($where);
	...
	//with group by condition LEFT JOIN AJAX
		function select_query_with_pagination( $data, $table_name, $where_condition =false, $limit=false, $offset=false, $all_record=false, $order_by_field=false, $order_by_type=false, $group_by_field = false, $join_tbl1=false, $join_type1=false, $join_condition1=false, $join_tbl2=false, $join_condition2=false, $join_type2=false, $join_tbl3=false, $join_condition3=false, $join_type3=false, $join_tbl4=false, $join_condition4=false, $join_type4=false, $join_tbl5=false, $join_condition5=false, $join_type5=false,$join_tbl6=false, $join_condition6=false, $join_type6=false, $join_tbl7=false, $join_condition7=false, $join_type7=false){
			
			$this->db->select( $data );
			$this->db->from($table_name);
			
			if(  !empty($join_condition1) ){
				$this->db->join( $join_tbl1, $join_condition1, $join_type1 );
			}
			if(  !empty($join_condition2) ){
				$this->db->join( $join_tbl2, $join_condition2, $join_type2 );
			}
			if(  !empty($join_condition3) ){
				$this->db->join( $join_tbl3, $join_condition3, $join_type3);
			}
			if(  !empty($join_condition4) ){
				$this->db->join( $join_tbl4, $join_condition4, $join_type4 );
			}
			if(  !empty($join_condition5) ){
				$this->db->join( $join_tbl5, $join_condition5, $join_type5 );
			}
			if(  !empty($join_condition6) ){
				$this->db->join( $join_tbl6, $join_condition6, $join_type6 );
			}
			if(  !empty($join_condition7) ){
				$this->db->join( $join_tbl7, $join_condition7, $join_type7);
			}
			if( !empty($offset) ){
				$this->db->limit($limit, $offset);
