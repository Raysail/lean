			...
			$setting_data = $this->user_model->get_row_with_con('tbl_admin',array('id'=>'1'));
			 if(!empty($all_message)){
						...
						?>
									...
									<?php $title_s = 'No'; if($review_list->sr_quality)	{$title_s = 'Yes'; } ?>
										<a href="#"><?php echo $title_s ;?></a>
											...
											<br /><br />  
										<p>
											<?php echo $review_list->sr_report ;?>
										</p>
            	...
            	<div class="authr-dshbrd-right">
					...
					<?php if( ($article_data[0]->art_status=='3') && ($msg_list->from_type=='A') && ($msg_list->again_send==0) )	{?>
							<div class="feed-box-btn">
								<form name="msg_send" id="mes_send_<?php echo $article_data[0]->art_no;?>" method="post" action="<?php echo base_url();?>send-message/<?php echo $article_data[0]->art_no;?>">
								...
								<a href="#" onclick="meg_submit('mes_send_<?php echo $article_data[0]->art_no;?>');">Send Message</a>
							</div>
								<?php }?>	
								
					<?php if( ($msg_list->art_status=='7') && ($msg_list->from_type=='A') && ($msg_list->again_send==0) )	{?>
							<div class="feed-box-btn">
								<form name="msg_send" id="mes_send_<?php echo $article_data[0]->art_no;?>" method="post" action="<?php echo base_url();?>send-message/<?php echo $article_data[0]->art_no;?>">
									<input type="hidden" name="msg_id" value="<?php echo $msg_list->id;?>" />
									<input type="hidden" name="art_no" value="<?php echo $article_data[0]->art_no;;?>" />
